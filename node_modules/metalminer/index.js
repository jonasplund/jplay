(function () {
    ...
    mm.getBandInfo = function (metaInfo, callback) {
        ...
        var callback2 = function (err, res) {
            ...
            currObj.status = 'DONE';
            currObj.data = res.data;
            currObj.error = res.error;
    ...
    var getMetalArchivesSimilarArtist = function (metaInfo, callback) {
        ...
        var that = this;
        var uri = 'http://www.metal-archives.com/search/ajax-advanced/searching/bands/?bandName=';
        uri += metaInfo.artist.replace(/ /gi, '+');
        request.get(uri, function (err, res, body) {
            ...
            if (body.aaData.length > 0 && body.aaData[0].length > 0) {
                ...
                request.get(uri2, function (err, res, body) {
                    ...
                    if (bands.length > 0) {
                        callback(null, { caller: that, bands: bands });
                    } else {
                        callback('No bands found.', { caller: that });
                    }
                });
            }
        });
