    ...
    setlist.setlistfm = function (metaInfo, callback) {
        ...
        request({ uri: uri }, function (err, res, body) {
            ...
            for (var i = 0; i < artists.length; i++) {
                if (artists[i]['@name'].toLowerCase() === metaInfo.artist.toLowerCase()) {
                    var mbid = artists[i]['@mbid'];
                    ...
                    request({ uri: uri }, function (err, res, body) {
                        ...
                        if (!body.setlists || !body.setlists.setlist || !body.setlists.setlist[0].sets) {
                            callback(undefined, { error: 'Invalid response: body.setlists(.setlist[0]/.sets) is falsy' });
                            return;
                        }
                        var setlist = [].concat(body.setlists.setlist)[0];
                        var sets = [].concat(setlist.sets)[0];
                        var set = [].concat(sets.set)[0];
                        callback(null, { data: set.song.map(function (curr) { return curr['@name']; }) });
                    });
                }
            }
        });
