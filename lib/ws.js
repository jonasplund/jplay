...
var sys = require("util"),
  ...
  handshakeTemplateXX = [
    'HTTP/1.1 101 Switching Protocols', // note a diff here
    //'Upgrade: websocket',
    'Connection: Upgrade',
    //'Sec-WebSocket-Origin: {origin}',
    'Sec-WebSocket-Location: {protocol}://{host}{resource}',
    ...
    return net.createServer(function (socket) {
      ...
      buffer = "";
        ...
        function handshake(data) {
            ...
            if (headers["sec-websocket-key"]) { // REAL FUCKING LIFE!
				console.log("reallife");
                var strkey = headers["sec-websocket-key"];
				var magic = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
                var hash = crypto.createHash("sha1");
				console.log(strkey + magic);
				hash.update(strkey + magic);
				var tmp = hash.digest();
				new Buffer(tmp, "binary").toString("base64");
				//tmp = tmp.toString("base64");
				console.log(tmp);
				socket.write(nano(handshakeTemplateXX, {
                    protocol: wsProtocol,
                    resource: data.get[1],
                    host: data.host[1],
