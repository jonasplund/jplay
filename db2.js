...
db.update = function () {
	...
	/*insertNew(options.musicDir, connection, function () {
		removeUnusedDirs(options.musicDir, connection, function () {
			removeUnusedSongs(options.musicDir, connection, function () {
    ...
    connection.query(qry, function (err, res) {
        ...
        for (var i = 0, endi = res.length; i < endi; i++) {
            ...
            if (!fs.existsSync(fullpath)) {
                ...
                connection.query("SELECT id FROM hashes WHERE hash = ?", hash, function (err2, res2) {
                if (res2.length > 0) {
                console.log("Found moved song. Updating hash reference.");
                connection.query("UPDATE hashes SET song_id = " + id + " WHERE id = " + res2[0].id, function (err) {
                if (err) throw err;
                });
                } else {
                }
                ...
                if (stat.isDirectory()) {
                    ...
                    connection.query(qry, fullpath, function (err, results) {
                        ...
                        if (results.length === 0) {
                            ...
                            insertDir(fullpath, stat, connection, function (tmp) {
...
var updateMusic = function (fullpath, stat, connection, callback) {
	...
	var title = id3.get("title"),
		...
		year = id3.get("year"),
		album = id3.get("album");
	qry = "SELECT * FROM songs WHERE hash = ?";
	connection.query(qry, hash, function (err, results) {
		...
		if (results.length === 0) {
			...
			if (fullpath !== fullpath2) {
				console.log("Paths didn't match. Updating: " + path.basename(fullpath));
				updateObj = {
...
/*var getHash = function (file) {
    ...
    fs.readSync(fd, buffer, 0, 16384, size);
    fs.closeSync(fd);
